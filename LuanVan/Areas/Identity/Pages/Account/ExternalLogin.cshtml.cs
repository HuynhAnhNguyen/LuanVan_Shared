...
namespace LuanVan.Areas.Identity.Pages.Account
{
    ...
    public class ExternalLoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetCallbackAsync(string returnUrl = null, string remoteError = null)
        {
            ...
            if (remoteError != null)
            {
                _notyf.Error(_localization.Getkey("LoiNhaCungCap") + " "+ remoteError);
                ErrorMessage = _localization.Getkey("LoiNhaCungCap") + " " + remoteError;
                return RedirectToPage("./Login", new { ReturnUrl = returnUrl });
            }
            var info = await _signInManager.GetExternalLoginInfoAsync();
            if (info == null)
            {
                _notyf.Error(_localization.Getkey("LoiTaiThongTin"));
                ErrorMessage = _localization.Getkey("LoiTaiThongTin");
                return RedirectToPage("./Login", new { ReturnUrl = returnUrl });
            }
            ...
            if (result.Succeeded)
            {
                _notyf.Success(info.Principal.Identity.Name + " "+ _localization.Getkey("DaDangNhapBang") + " " + info.LoginProvider);
                _logger.LogInformation("{Name} logged in with {LoginProvider} provider.", info.Principal.Identity.Name, info.LoginProvider);
                return LocalRedirect(returnUrl);
            }
            if (result.IsLockedOut)
            {
                _notyf.Error(_localization.Getkey("TaiKhoanBiKhoa"));
        ...
        public async Task<IActionResult> OnPostConfirmationAsync(string returnUrl = null)
        {
            ...
            if (info == null)
            {
                _notyf.Error(_localization.Getkey("LoiTaiThongTinBenNgoai"));
                ErrorMessage = _localization.Getkey("LoiTaiThongTinBenNgoai");
                return RedirectToPage("./Login", new { ReturnUrl = returnUrl });
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                user.HoKhachHang = info.Principal.Identity.Name;
                ...
                if (result.Succeeded)
                {
                    ...
                    if (result.Succeeded)
                    {
                        _notyf.Success(_localization.Getkey("TaoTaiKhoanThanhCongBang") + " "+ info.LoginProvider);
